...
Do **all** of this **on the Ablaze VM**. Your laptop is only the browser. Do **not** run `oc login` (ARO stays idle until Lab 4). Copy **one block at a time**. Do not paste two commands on the same line (`curl.exe --version` and `cd` must be separate). Do **not** paste this whole guide (or a chat) into the terminal.
