...
| ImagePullBackOff and Events say **layer does not match config's DiffID** | Pullspec is correct (internal registry). The first Python push double-gzipped Docker Desktop layers. Step 0 `git pull`, then re-run `tools\push-images.ps1`. Scale the two Deployments to `0` then `1` to drop ReplicaSets stuck on the bad image. |
| `Missing statement body in do loop` / parse errors in `push-to-openshift.ps1` (`Do not run docker login...`) | Old helper used here-strings that Ablaze Windows PowerShell misreads. Stash filled YAML if needed (row below), `git pull` from `%USERPROFILE%\MD287`, then re-run `push-images.ps1`. |
